...
<script lang="ts">
    ...
    import { allergies } from '../store.js';
...
<main class="flex flex-col h-screen w-screen text-center items-center">
    <button>
        <Link to="/cupboard" on:click={()=> allergies.set(Object.keys(names))}>
            <img src="/assets/arrow_forward_black_24dp.svg" alt="arrow" class="w-10 h-auto absolute right-7 mt-5 cursor-pointer">
        </Link>
    </button>
    <h1 class="mt-24 font-bold text-2xl pb-8">Are You Allergic To Any Products?</h1>
